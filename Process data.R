...
for (recipe.num in 2:dim(d)[1]) { #dim(d)[1] ultimately i'll include every recipe (i think), but while developing, a small subset is sufficient
...
Find.S <- function(elements.to.subset = 1:dim(B)[1]) {
  ...
  dominant_u <-  power.iter(new.B, rep(1,nrow(new.B)))
